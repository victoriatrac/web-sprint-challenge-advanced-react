...
~ Stateful components have state and keep track of data as it updates. Functional components, or stateless components, don't maintain state and are getting data from props or always render the same data.

2. When does a componentWillMount function be called? What about a componentWillUpdate?
~ ComponentWillMount should be called before rendering initial data. ComponentWillUpdate should be called after data updates but before rendering of the new data.
...
~ Arrange Act Assert.
Arrange - renders react element into virtual dom
Act - searchs DOM for content in browser
Assert - checks that content has rendered